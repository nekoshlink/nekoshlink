...
open class DefaultShortUrlManager(
    private val dao: ShortUrlDataAccess,
    private val shortener: UrlShortener,
    private val shortCodeValidator: ShortCodeValidator,
    ...
    override fun create(meta: ShortUrlCreateMeta, options: ShortUrlCreateOptions): ShortUrl {
        ...
        if (!customSlugWasProvided) {
            meta.shortCode = shortener.shorten(options.shortCodeLength)
        }
        ...
        shortUrl.titleWasAutoResolved = if (options.validateUrl) {
            if (options.useValidatedTitle && shortUrl.title == null) {
                val resolvedTitle = urlValidator.validateWithTitle(shortUrl.longUrl) ?: throw InvalidLongUrlException(shortUrl.longUrl)
                shortUrl.title = resolvedTitle
                true
            } else {
                if (urlValidator.validate(shortUrl.longUrl)) {
        ...
        if (!shortCodeValidator.validate(shortUrl.shortCode)) {
    ...
    override fun resolve(meta: ResolveMeta, enricher: VisitDataEnricher): Pair<ShortUrl?, UriComponents?> {
        ...
        val shortUrl = RunAsBlock.userWithRoles("ShortUrls:Viewer").use {
